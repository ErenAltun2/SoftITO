...
--gruplama ve toplama fonksıyonları 
...
-- CASE 
SELECT ad,puan,
	CASE WHEN puan >= 85 THEN 'AA' WHEN puan >= 70 THEN 'BB' ELSE 'ff' END AS harf
	FROM notlar JOIN ogrenciler ON ogrenciler.id = notlar.ogrenci_id;
	
--İNDEX 
CREATE INDEX idx_ogrenciler_ad ON ogrenciler(ad);
CREATE UNIQUE INDEX idx_eposta ON ogrenciler (eposta); 
